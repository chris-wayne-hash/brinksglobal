...
function startTrackingAnimation() {
    ...
    setTimeout(() => {
        originNode.classList.add("active");
        updateTrackingInfo("Gold shipment confirmed and processed", "5-7 business days");
        showToast("Arrived - Le Havre (FR), in Bond", "info");
    }, 1000);
}

function resetRoadmap() {
    const originNode = document.querySelector('.origin-node');
    originNode.classList.remove("active", "completed");
    
    // Reset animation
    const nodeCircle = originNode.querySelector('.node-circle');
    if (nodeCircle) {
